                    ...
                    switch (snipeType) { // Use the snipe type we select
                        case '1':
                            if (reDX.test(tx.data)) { // Search the mempool for the selected method
                                BuyToken(tx) // Buy the token and pass on the transaction
                            }
                            break
...
async function tradeTokensForExactETHWithSupportingFee(
    ...
    try {
        sleep(argv.sell * 1000) // Wait x time before selling
...
async function approveToken(tokenToPurchase) {
    try {
